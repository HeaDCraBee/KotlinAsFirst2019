...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((name, grade) in grades) {
        res[grade] = res.getOrPut(grade) { listOf() } + name
    }
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    val mapAll = mapA.toMutableMap()
    for ((key, value) in mapB) {
        if (key !in mapAll) mapAll[key] = value
        if (mapAll[key] != mapB[key]) mapAll[key] += ", ${mapB[key]}"
    }
    return mapAll
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    val sum = mutableMapOf<String, List<Double>>()
    for ((key, value) in stockPrices)
        sum[key] = sum.getOrDefault(key, listOf()) + value

    for ((key, value) in sum)
        res[key] = value.average()
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val charSet = chars.toString().toLowerCase()
